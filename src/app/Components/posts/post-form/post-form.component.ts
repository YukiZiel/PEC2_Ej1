...
export class PostFormComponent implements OnInit {
  ...
  private async loadCategories(): Promise<void> {
    ...
    if (userId) {
      try {
        this.categoryService.getCategoriesByUserId(userId).subscribe((categoriesLists) => {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (this.postId) {
      ...
      try {
        this.postService.getPostById(this.postId).subscribe((postsById) => {
          this.post = postsById;
        });
  ...
  private createPost(): Observable<boolean> {
    ...
    return from(this.postService.createPost(this.post)).pipe(
      mapTo(true),
      catchError((error) => {
      ...
      switchMap((responseOK) => {
        if (typeof responseOK === 'boolean') {
        return this.sharedService.managementToast('postFeedback', responseOK);
      } else {
        return of(false);
      }
      })
  ...
  async savePost() {
    ...
    if (this.isUpdateMode) {
      ...
      this.createPost().subscribe((postCreate) => {
        this.validRequest = postCreate;
      })
    }
  }
}
